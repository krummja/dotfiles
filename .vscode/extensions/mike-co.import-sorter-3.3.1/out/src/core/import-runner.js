...
const helpers_public_2 = require("./helpers/helpers-public");
class SimpleImportRunner {
    ...
    getExcludeUnusedImports(sortResult, usedTypeReferences) {
        ...
        const sortResultClonned = lodash_1.cloneDeep(sortResult);
    ...
    sortAllImports$(startingSourcePath) {
        ...
        return allFilePaths$.pipe(operators_1.mergeAll(), operators_1.flatMap(path => this.sortFileImports$(path), 3));
    }
    sortFileImports$(fullFilePath) {
        return helpers_public_1.io.readFile$(fullFilePath).pipe(operators_1.switchMap(file => {
    ...
    getFullSortedSourceFile(sourceText, sortedData) {
        ...
        const linesToDelete = lodash_1.chain(sortedData.rangesToDelete.map(range => lodash_1.range(range.startLine, range.endLine)))
    ...
    allFilePathsUnderThePath$(startingSourcePath) {
        ...
        return rxjs_1.merge(...filesPaths$);
    }
    ...
    getRangesToDelete(sortedImportsResult, fileSourceArray, fileSourceText) {
        const sortedImports = lodash_1.chain(sortedImportsResult.groups).flatMap(x => x.elements).value();
        const rangesToDelete = [];
        lodash_1.chain(sortedImports)
            ...
            const startPosition = firstLeadingComment ? helpers_public_2.textProcessing.getPositionByOffset(firstLeadingComment.range.pos, fileSourceText) : x.startPosition;
            const endPosition = lastTrailingComment ? helpers_public_2.textProcessing.getPositionByOffset(lastTrailingComment.range.end, fileSourceText) : x.endPosition;
